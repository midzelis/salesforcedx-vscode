...
async function registerCommands(activateDX: boolean): Promise<vscode.Disposable | undefined> {
    try {
        await waitForDX(activateDX);
        const { forceLightningLwcCreate } = require('./commands/forceLightningLwcCreate');

        // Customer-facing commands
        const forceLightningLwcCreateCmd = vscode.commands.registerCommand('sfdx.force.lightning.lwc.create', forceLightningLwcCreate);
...
export async function activate(context: vscode.ExtensionContext) {
    ...
    const workspaceType = lwcLanguageServer.detectWorkspaceType(vscode.workspace.workspaceFolders[0].uri.fsPath);
    ...
    if (workspaceType === lwcLanguageServer.WorkspaceType.SFDX) {
        await populateEslintSettingIfNecessary(context, vscode.workspace.getConfiguration('', vscode.workspace.workspaceFolders[0].uri));
    }
...
function startLWCLanguageServer(serverModule: string, context: vscode.ExtensionContext) {
    ...
    const clientOptions: LanguageClientOptions = {
        documentSelector: [{ language: 'html', scheme: 'file' }, { language: 'javascript', scheme: 'file' }],
        synchronize: {
            fileEvents: [
                vscode.workspace.createFileSystemWatcher('**/*.resource'),
                vscode.workspace.createFileSystemWatcher('**/labels/CustomLabels.labels-meta.xml'),
                vscode.workspace.createFileSystemWatcher('**/staticresources/*.resource-meta.xml'),
                vscode.workspace.createFileSystemWatcher('**/contentassets/*.asset-meta.xml'),
    ...
    const disposable = new LanguageClient('lwcLanguageServer', 'LWC Language Server', serverOptions, clientOptions).start();
    // Push the disposable to the context's subscriptions so that the
    // client can be deactivated on extension deactivation
    context.subscriptions.push(disposable);
}

export async function populateEslintSettingIfNecessary(context: vscode.ExtensionContext, config: vscode.WorkspaceConfiguration) {
    ...
    if (!nodePath || nodePath.includes(LWC_EXTENSION_NAME)) {
        ...
        await config.update(ESLINT_NODEPATH_CONFIG, eslintModule, vscode.ConfigurationTarget.Workspace);
    }
}
